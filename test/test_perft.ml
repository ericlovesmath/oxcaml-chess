...
let table fen ~depths =
  ...
  List.init depths ~f:(fun i ->
    ...
    [%sexp
      { depth : int
      ; nodes = (counts.nodes : int)
      ; captures = (counts.captures : int)
      ; en_passant = (counts.en_passant : int)
      ; castles = (counts.castles : int)
      ; promotions = (counts.promotions : int)
      ; checks = (counts.checks : int)
      ; checkmates = (counts.checkmates : int)
      }])
  |> Expectable.print
;;
...
let%expect_test "perft from startpos" =
  ...
  [%expect
    {|
    ┌───────┬─────────┬──────────┬────────────┬─────────┬────────────┬────────┬────────────┐
    │ depth │ nodes   │ captures │ en_passant │ castles │ promotions │ checks │ checkmates │
    ├───────┼─────────┼──────────┼────────────┼─────────┼────────────┼────────┼────────────┤
    │ 1     │      20 │     0    │   0        │ 0       │ 0          │     0  │   0        │
    │ 2     │     400 │     0    │   0        │ 0       │ 0          │     0  │   0        │
